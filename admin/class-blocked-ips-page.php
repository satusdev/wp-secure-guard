...
final class Secure_Guard_Blocked_IPs_Page {
    ...
    public function handle_bulk_unblock(): void {
        ...
        $mode = sanitize_key((string) ($_POST['bulk_mode'] ?? 'all'));
    ...
    public function render(): void {
        ...
        if (!empty($_GET['login_unblocked'])) {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Login lockout cleared successfully.', 'secure-guard') . '</p></div>';
        }
        if (isset($_GET['bulk_unblocked'])) {
            echo '<div class="notice notice-success is-dismissible"><p>' . sprintf(esc_html__('%d selected block(s) processed.', 'secure-guard'), (int) $_GET['bulk_unblocked']) . '</p></div>';
        }
        if (isset($_GET['login_bulk_unblocked'])) {
            echo '<div class="notice notice-success is-dismissible"><p>' . sprintf(esc_html__('%d login lockout(s) cleared.', 'secure-guard'), (int) $_GET['login_bulk_unblocked']) . '</p></div>';
        }

        // ── quick unlock tool ───────────────────────────────────────────
        echo '<div class="card" style="max-width:900px;padding:16px;margin-bottom:16px;">';
        echo '<h2 style="margin-top:0;font-size:16px;">' . esc_html__('Quick Unlock', 'secure-guard') . '</h2>';
        echo '<p class="description">' . esc_html__('Enter an IP address to immediately clear all associated blocks and reset their failure count.', 'secure-guard') . '</p>';
        ...
        submit_button(__('Unlock IP', 'secure-guard'), 'primary', 'submit', false);
        echo '</form></div>';

        echo '<div class="card" style="max-width:900px;padding:16px;margin-bottom:16px;">';
        echo '<h2 style="margin-top:0;font-size:16px;">' . esc_html__('False Positive Recovery', 'secure-guard') . '</h2>';
        echo '<p class="description">' . esc_html__('If real admins are locked out after failed logins, clear only login lockouts first. Firewall blocks remain in place.', 'secure-guard') . '</p>';
        ...
        submit_button(__('Clear All Login Lockouts', 'secure-guard'), 'secondary', 'submit', false, ['onclick' => 'return confirm(\'' . esc_js(__('Clear every active login lockout?', 'secure-guard')) . '\')']);
        echo '</form>';
        echo '<a class="button" href="' . esc_url(admin_url('admin.php?page=secure-guard-assistant')) . '">' . esc_html__('Open Security Assistant', 'secure-guard') . '</a>';
        echo '</div>';

        echo '<div class="card" style="max-width:900px;padding:16px;">';
        echo '<p style="margin-top:0;color:#646970;">' . sprintf(esc_html__('%d active block(s) detected', 'secure-guard'), $total) . '</p>';
        ...
        echo '<option value="all">' . esc_html__('Unblock selected IPs completely', 'secure-guard') . '</option>';
        echo '<option value="login-only">' . esc_html__('Clear selected login lockouts only', 'secure-guard') . '</option>';
        echo '</select>';
        submit_button(__('Apply to Selected', 'secure-guard'), 'secondary', 'submit', false, ['onclick' => 'return confirm(\'' . esc_js(__('Apply the selected unblock action?', 'secure-guard')) . '\')']);
        echo '</form>';

        echo '<table class="widefat striped">';
        echo '<thead><tr>';
        echo '<th style="width:36px;"><input type="checkbox" class="sg-select-all" aria-label="' . esc_attr__('Select all active blocks', 'secure-guard') . '" /></th>';
        echo '<th>' . esc_html__('IP Address', 'secure-guard') . '</th>';
        echo '<th>' . esc_html__('Type', 'secure-guard') . '</th>';
        echo '<th>' . esc_html__('Why Blocked', 'secure-guard') . '</th>';
        echo '<th>' . esc_html__('Blocked Since (UTC)', 'secure-guard') . '</th>';
        echo '<th>' . esc_html__('Expires (UTC)', 'secure-guard') . '</th>';
        echo '<th style="width:100px;">' . esc_html__('Action', 'secure-guard') . '</th>';
        echo '</tr></thead>';
        echo '<tbody>';
        ...
        foreach ($blocks as $row) {
            ...
            if ($reasons === []) {
                echo esc_html__('No related log retained.', 'secure-guard');
            } else {
            ...
            echo '</form>';
            if ($type === 'Login') {
                echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '" onsubmit="return confirm(\'' . esc_js(__('Clear only this login lockout?', 'secure-guard')) . '\')">';
                echo '<input type="hidden" name="action" value="sg_unblock_login_ip" />';
                echo '<input type="hidden" name="ip" value="' . esc_attr($ip) . '" />';
                wp_nonce_field('sg_unblock_login_ip');
                submit_button(__('Login only', 'secure-guard'), 'secondary small', '', false);
                echo '</form>';
            }
            echo '</div>';
            echo '</td>';
