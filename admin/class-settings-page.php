...
final class Secure_Guard_Settings_Page {
    ...
    public function render(): void {
        ...
        <div class="wrap secure-guard-ui">
            ...
            <p class="description"><?php echo esc_html__('Configure REST lock policy, endpoint protections, rate limits, and hardening defaults.', 'secure-guard'); ?></p>
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table" role="presentation">
                    <tr><th><?php echo esc_html__('REST Lock', 'secure-guard'); ?></th><td><label><input type="checkbox" name="<?php echo esc_attr(Secure_Guard_Config::OPTION_KEY); ?>[rest_lock_enabled]" value="1" <?php checked(!empty($settings['rest_lock_enabled'])); ?> /> <?php echo esc_html__('Enable REST API default-deny', 'secure-guard'); ?></label></td></tr>
                    <tr><th><?php echo esc_html__('Block Sensitive Endpoints', 'secure-guard'); ?></th><td><label><input type="checkbox" name="<?php echo esc_attr(Secure_Guard_Config::OPTION_KEY); ?>[block_sensitive_endpoints]" value="1" <?php checked(!empty($settings['block_sensitive_endpoints'])); ?> /></label></td></tr>
                    <tr><th><?php echo esc_html__('Block User Enumeration', 'secure-guard'); ?></th><td><label><input type="checkbox" name="<?php echo esc_attr(Secure_Guard_Config::OPTION_KEY); ?>[block_user_enumeration]" value="1" <?php checked(!empty($settings['block_user_enumeration'])); ?> /></label></td></tr>
                    <tr><th><?php echo esc_html__('Block XML-RPC', 'secure-guard'); ?></th><td><label><input type="checkbox" name="<?php echo esc_attr(Secure_Guard_Config::OPTION_KEY); ?>[block_xmlrpc]" value="1" <?php checked(!empty($settings['block_xmlrpc'])); ?> /></label></td></tr>
                    ...
                    <tr><th><?php echo esc_html__('Content Security Policy', 'secure-guard'); ?></th><td><input type="text" class="regular-text" name="<?php echo esc_attr(Secure_Guard_Config::OPTION_KEY); ?>[csp]" value="<?php echo esc_attr((string) $settings['csp']); ?>" /></td></tr>
